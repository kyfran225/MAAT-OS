...
export const MissionsView: React.FC<MissionsViewProps> = ({
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      {missions.length === 0 ? (
        <div className="glass-panel p-12 rounded-3xl text-center space-y-4 max-w-2xl mx-auto">
          ...
          <h2 className="text-xl font-bold text-white">Aucune Mission pour votre entreprise</h2>
          <p className="text-xs text-slate-400 leading-relaxed">
            Créez votre première Mission Métier. Définissez votre objectif commercial ou stratégique, et le Moteur Cognitif décomposera la mission en 6 couches opérationnelles pour votre équipe d'agents.
          </p>
          <button
            onClick={onOpenCreateModal}
            className="px-6 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 font-bold text-xs hover:brightness-110 shadow-lg shadow-amber-500/20 inline-flex items-center gap-2"
          >
            <PlusCircle className="w-4 h-4 stroke-[3]" />
            <span>Lancer ma première Mission</span>
          </button>
        </div>
      ) : (
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="space-y-3">
              {filteredMissions.map((mission) => {
                const isSelected = mission.id === (selectedMission?.id || selectedMissionId);
                return (
                  <div
                    key={mission.id}
